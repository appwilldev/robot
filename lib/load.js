...
exports.load = function (path) {
    ...
    fs.exists(path, function(exists) {
        ...
        fs.readFile(path, 'UTF-8', function(err, script) {
            ...
            // root
            var options = {
                hostname: conf.url.slice(0, conf.url.indexOf('/', 0) != -1 ? conf.url.indexOf('/', 0) : conf.url.length),
                path: conf.url.indexOf('/', 0) != -1 ? conf.url.slice(conf.url.indexOf('/', 0), conf.url.length) : '/',
                method: 'GET'
            };
            request = http.request(options, function(res) {
                ...
                res.on('data', function(chunk) {
                    ...
                    fetch.analyse(data, root, conf);
                    // utils.filter(conf, media);
                });
            });
        });
    });
...
function element(node) {
    if (node == null || node == '' || node.html() == undefined)
        return null;
    var tmp = res = {'tagName': null, 'attr':{}, 'index': -1, 'next': null};
    var target = null;
    try {
        if (node.length >= 0){
            var t = node.text().trim();
            if (t != '')
                target = t.match(/\[.*\]/g)[0].slice(1, -1);
            node = node.children();
        }
    }
    catch (err) {
    ...
    while(node.length > 0) {
        ...
        node = node.children();
        if(node.length > 0){
            tmp.next = {'tagName': null, 'attr':{}, 'index': -1, 'next': null};
            tmp = tmp.next;
        }
    }
    tmp['target'] = target;
