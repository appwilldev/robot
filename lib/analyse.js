...
function element(node) {
    if (node == null || node == '' || typeof node.html() == 'undefined') 
        return null
    // {tag:{key:value,key: value}, next: null} 
    print(node.find(':first').attr('id'));
    print(node.find(':first').get(0).tagName);
    $(node.find(':first').get(0).attributes).each(function() {
